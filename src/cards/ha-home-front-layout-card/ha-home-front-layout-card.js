...
class HaHomeDesktopLayoutCard extends HTMLElement {
  ...
  disconnectedCallback(){window.removeEventListener("resize",this._resize);window.visualViewport?.removeEventListener("resize",this._resize);this._unbindBreakpoints();this._ro?.disconnect();this._ro=null;cancelAnimationFrame(this._fillRaf);}
  ...
  async _build(){
    ...
    this._fill=0;this._observeMobile();this._applyLayoutVars();this._applyMode();this._flushActive();this._fitViewport();
  }
  ...
  // som indholdet ændrer højde - grafen tilpasser sig uden synligt hop. Kun
  // header og kortene over det sidste observeres, så fyldet ikke udløser sig selv.
  _observeMobile(){const mobile=this.shadowRoot?.querySelector(".mobile"),header=this.shadowRoot?.querySelector(".header");if(!mobile||!window.ResizeObserver)return;this._ro?.disconnect();this._ro=new ResizeObserver(()=>this._fitMobile());if(header)this._ro.observe(header);[...mobile.children].slice(0,-1).forEach((slot)=>this._ro.observe(slot));}
  _scheduleFill(){cancelAnimationFrame(this._fillRaf);this._fillRaf=requestAnimationFrame(()=>this._fitMobile());}
  _navTop(){if(!this._navEl?.isConnected){this._navEl=null;const nav=this.getRootNode?.()?.querySelector?.("ha-navbar-card");const walk=(root,depth)=>{if(!root||depth>6)return null;for(const el of root.children){if(getComputedStyle(el).position==="fixed"&&el.getBoundingClientRect().height>0)return el;const hit=walk(el.shadowRoot,depth+1)||walk(el,depth+1);if(hit)return hit;}return null;};this._navEl=walk(nav?.shadowRoot,0);}const rect=this._navEl?.getBoundingClientRect();return rect&&rect.height>0?rect.top:null;}
  _fitMobile(){const mobile=this.shadowRoot?.querySelector(".mobile"),surface=this.shadowRoot?.querySelector(".surface"),last=mobile?.lastElementChild;if(!last||!surface)return;const desktop=window.matchMedia(`(min-width:${(this._mobileBreakpoint||1100)+1}px)`).matches;const navTop=desktop?null:this._navTop();let next=0;if(navTop!=null){const gap=Math.max(0,Number(this.config?.mobile_nav_gap??8));const bottom=surface.getBoundingClientRect().bottom+window.scrollY-this._fill;next=Math.max(0,Math.min(400,Math.round(navTop-gap-bottom)));}if(Math.abs(next-this._fill)<3)return;this._fill=next;if(next)last.style.setProperty("--front-mobile-fill",`${next}px`);else last.style.removeProperty("--front-mobile-fill");}
