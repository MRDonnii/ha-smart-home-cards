...
class HaHomeDesktopLayoutCard extends HTMLElement {
  constructor(){super();this.attachShadow({mode:"open"});this._children=[];this._headerChildren=[];this._desktopChildren=[];this._mobileChildren=[];this._verticalChildren=[];this._mqList=[];this._resize=()=>requestAnimationFrame(()=>this._fitViewport());this._onBreakpoint=()=>this._flushActive();this._slots={header_cards:[],left_cards:[],right_cards:[],mobile_cards:[],vertical_cards:[]};this._slotSig={header_cards:[],left_cards:[],right_cards:[],mobile_cards:[],vertical_cards:[]};this._mode="cards";this._fill=0;this._fillRaf=0;this._navEl=null;this._ro=null;}
  _applyMode(){
  ...
  connectedCallback(){window.addEventListener("resize",this._resize);window.visualViewport?.addEventListener("resize",this._resize);this._observeMobile();this._resize();this._flushActive();}
  disconnectedCallback(){window.removeEventListener("resize",this._resize);window.visualViewport?.removeEventListener("resize",this._resize);this._unbindBreakpoints();this._ro?.disconnect();this._ro=null;cancelAnimationFrame(this._fillRaf);clearTimeout(this._navTimer);}
  ...
  set hass(hass){this._hass=hass;const mode=hass?.states?.["input_boolean.dashboard_samlet_forside"]?.state==="on"?"unified":"cards";if(mode!==this._mode){this._mode=mode;this._applyMode();}this._activeChildren().forEach((card)=>{card.hass=hass;});}
  async _build(){
    ...
    this._fill=0;this._navTries=0;this._observeMobile();this._applyLayoutVars();this._applyMode();this._flushActive();this._fitViewport();
  }
  // Mobil: lader sidste kort (strømprisgrafen) vokse via --front-mobile-fill, så
  // afstanden fra fladens bund til den faste navbar svarer til afstanden i toppen.
  // Kun når siden i forvejen er kortere end skærmen; ellers er fyldet 0.
  // ResizeObserver-kaldet kører før næste paint, så fyldet sættes i samme frame
  // som indholdet ændrer højde - grafen tilpasser sig uden synligt hop. Alle mål er
  // layout-mål (offsetTop/offsetHeight), så animationer og scroll ikke kan forskyde
  // resultatet; et fyld der allerede passer, ændres ikke igen.
  _observeMobile(){const mobile=this.shadowRoot?.querySelector(".mobile"),header=this.shadowRoot?.querySelector(".header");if(!mobile||!window.ResizeObserver)return;this._ro?.disconnect();this._ro=new ResizeObserver(()=>this._fitMobile());if(header)this._ro.observe(header);[...mobile.children].forEach((slot)=>this._ro.observe(slot));}
  _scheduleFill(){cancelAnimationFrame(this._fillRaf);this._fillRaf=requestAnimationFrame(()=>this._fitMobile());}
  // Navbarens faste plads i bunden (højde + CSS-bund). offsetHeight og den beregnede
  // bund påvirkes ikke af animationer eller scroll, så resultatet er stabilt.
  _navSpace(){if(!this._navEl?.isConnected){this._navEl=null;const nav=this.getRootNode?.()?.querySelector?.("ha-navbar-card");const walk=(root,depth)=>{if(!root||depth>6)return null;for(const el of root.children){if(getComputedStyle(el).position==="fixed"&&el.offsetHeight>0)return el;const hit=walk(el.shadowRoot,depth+1)||walk(el,depth+1);if(hit)return hit;}return null;};this._navEl=walk(nav?.shadowRoot,0);}const el=this._navEl;if(!el||!el.offsetHeight)return null;return el.offsetHeight+(parseFloat(getComputedStyle(el).bottom)||0);}
  // Fladens top i dokumentet via offsetTop-kæden (ignorerer transform og scroll).
  _staticTop(el){let top=0,node=el,guard=0;while(node&&guard++<80){top+=node.offsetTop||0;node=node.offsetParent;}return top;}
  _fitMobile(){const mobile=this.shadowRoot?.querySelector(".mobile"),surface=this.shadowRoot?.querySelector(".surface"),last=mobile?.lastElementChild;if(!last||!surface)return;const desktop=window.matchMedia(`(min-width:${(this._mobileBreakpoint||1100)+1}px)`).matches;let next=0;if(!desktop){const nav=this._navSpace();if(nav==null){if((this._navTries=(this._navTries||0)+1)<12){clearTimeout(this._navTimer);this._navTimer=setTimeout(()=>this._fitMobile(),500);}return;}const gap=Math.max(0,Number(this.config?.mobile_nav_gap??8));const natural=surface.offsetHeight-this._fill;next=Math.max(0,Math.min(600,Math.round(window.innerHeight-nav-gap-this._staticTop(surface)-natural)));}if(Math.abs(next-this._fill)<2)return;this._fill=next;if(next)last.style.setProperty("--front-mobile-fill",`${next}px`);else last.style.removeProperty("--front-mobile-fill");}
  _fitViewport(){this._scheduleFill();const layout=this.shadowRoot?.querySelector(".layout");if(!layout||!window.matchMedia(`(min-width:${(this._mobileBreakpoint||1100)+1}px)`).matches)return;const top=layout.getBoundingClientRect().top;const viewportHeight=window.visualViewport?.height||window.innerHeight;const bottomGap=Math.max(90,Number(this.config?.bottom_gap)||110);layout.style.setProperty("--desktop-height",`${Math.max(0,viewportHeight-top-bottomGap)}px`);}
