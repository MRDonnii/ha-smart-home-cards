...
class HaHomeRoomOverviewCard extends HTMLElement {
  ...
  _roomState(room) {
    ...
    const alert = room.alert
      ? (room.alert.above != null ? (this._number(room.alert.entity) ?? 0) > room.alert.above : this._active(room.alert.entity))
      : false;
    const humidityCritical = humidity != null && !room.outdoor && humidity >= (room.humidity_critical ?? 70);
    return { temp, humidity, light, presence, open, alert, humidityCritical };
  }
  ...
  async _loadHistory() {
    ...
    if (this._historyLoadedAt && now - this._historyLoadedAt < 600000) return;
    ...
    finally { this._historyLoading = false; this._historyLoadedAt = Date.now(); this._render(); }
  }
  ...
  _render() {
    ...
    requestAnimationFrame(() => {
      this.shadowRoot.querySelectorAll(".status,.room-note").forEach((el) => { el.style.opacity = "1"; });
    });
  }
