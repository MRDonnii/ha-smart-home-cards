...
const HAHomeWeatherFx = (() => {
  ...
  function density(perPixels, cap) {
    return Math.min(cap, Math.max(4, Math.round(((width * height) / perPixels) * densityScale)));
  }
  ...
  const LAYER_BUILDERS = {
    ...
    rain: () => rainDrops(density(6500, 190), 8, 7, 15, 20, 0.34, 0.28),
  ...
  function drawParticle(p) {
    ...
    if (p.kind === "rain") {
      ctx.globalAlpha = p.opacity;
      ctx.strokeStyle = "rgba(205,228,255,1)"; ctx.lineWidth = 1.8;
  ...
  function applyCondition(condition) {
    ...
    if (canvas) canvas.dataset.condition = condition || "none";
  ...
  function mount() {
    ...
    canvas.id = "haHomeWeatherFx";
    canvas.setAttribute("aria-hidden", "true");
    canvas.style.cssText = "position:fixed;inset:0;width:100%;height:100%;z-index:0;pointer-events:none;display:none";
  ...
  return {
    setCondition(owner, condition, active = true, options = {}) {
      mount();
      currentOwner = owner;
      const intensity = Math.min(1, Math.max(0.1, Number(options.intensity) || 0.42));
      densityScale = Math.min(1.5, Math.max(0.2, Number(options.density) || 0.6));
      const layer = options.layer === "over" ? "over" : "behind";
      const nextOptionsKey = `${intensity}:${densityScale}:${layer}`;
      if (nextOptionsKey !== optionsKey) currentCondition = null;
      optionsKey = nextOptionsKey;
      canvas.style.opacity = String(intensity);
      canvas.style.zIndex = layer === "over" ? "999" : "0";
      applyCondition(active && !reducedMotion ? condition : null);
      if (canvas) canvas.style.display = activeConfig ? "block" : "none";
    },
    clear(owner) {
...
window.HAHomeWeatherFx = HAHomeWeatherFx;
