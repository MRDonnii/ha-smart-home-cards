...
class HAHeatingDiagnosticsCard extends HTMLElement {
  ...
  _state(room) {
    ...
    else if (stressed) { level = "problem"; title = "Radiator belastet"; detail = "Ventilen står fuldt åben, men rummet når ikke sit mål. Tjek om radiatoren er luftet, eller om den er for lille til rummet."; }
    else if (coldUnderLoad) { level = "problem"; title = "Koldt trods åben ventil"; detail = "Rummet er koldere end målet, selv om ventilen er højt åben. Tjek for kuldetræk fra vindue/dør, eller om ventilen sidder fast."; }
    else if (notResponding) { level = "problem"; title = "Ingen effekt fra varmen"; detail = "Lært varmeeffekt matcher næsten rummets varmetab - radiatoren giver muligvis ikke reel varme. Tjek fremløbet til rummet, og om ventilen faktisk åbner."; }
    else if (demandStatus === "deviating" && baselineReady) { level = "warn"; title = "Afviger fra normalen"; detail = deviation === undefined ? "Vejrkorrigeret varmebehov ligger uden for rummets lærte mønster" : deviation > 0 ? `Bruger ${this._format(deviation,0)}% mere varme end vejrkorrigeret normalt` : `Bruger ${this._format(Math.abs(deviation),0)}% mindre varme end vejrkorrigeret normalt`; }
