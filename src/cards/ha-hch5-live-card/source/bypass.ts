...
export function bypassTravel({ raw, requestOn, direction, seconds, total, observed = null }: {
  ...
  if (raw <= 0 || raw >= 255) {
    ...
    return requestOn && raw <= 0 ? { direction: "opening", percent: 0, remainingSeconds: travelSeconds } : null;
  }
  ...
  if (elapsed === null) return { direction: heading, percent: null, remainingSeconds: null };
  return {
    direction: heading,
    percent: Math.min(99, Math.round((elapsed / travelSeconds) * 100)),
    remainingSeconds: Math.max(0, Math.round(travelSeconds - elapsed)),
  };
}
