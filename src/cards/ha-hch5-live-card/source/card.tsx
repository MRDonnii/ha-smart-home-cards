  ...
  return <div className="hch-smartdash">
    ...
    <div className="hch-smartdash-body">
      ...
      <div className="hch-smartdash-controls" onClick={event => event.stopPropagation()}>
        ...
        <div className="hch-smartdash-control-row hch-smartdash-level"><div className="hch-smartdash-control-label"><span>Ventilatorniveau</span><em>Aktuelt trin {num("effective_level") ?? "—"}</em></div><div className="hch-smartdash-choice">{[1,2,3,4,5,6].map(level => <button key={level} type="button" aria-pressed={Number(selected(levelKey) ?? chosenLevel) === level} disabled={busy} onClick={() => void command(levelKey, mode === "manual" ? "select" : "number", mode === "manual" ? "select_option" : "set_value", mode === "manual" ? { option: String(level) } : { value: level }, String(level))}>{level}</button>)}</div></div>
