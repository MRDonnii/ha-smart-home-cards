...
const PRESETS = {
  home_energy: {
    ...
    vehicle_power_entity: "sensor.ev_charger_power",
    phase_entities: [
      "sensor.home_phase_a_power",
      "sensor.home_phase_b_power",
      "sensor.home_phase_c_power",
    ],
    phase_max: 3000,
    daily_entity: "sensor.home_energy_today",
  ...
  ev: {
    ...
    phase_entities: [
      "sensor.ev_charger_current_phase_1",
      "sensor.ev_charger_current_phase_2",
      "sensor.ev_charger_current_phase_3",
    ],
    ...
    navigation_path: "/lovelace/vehicle",
    value_unit: "%",
  },
  electricity_price: {
    name: "Strømpris",
    ...
    navigation_path: "/lovelace/energy-prices",
    value_unit: "kr",
  },
  pool: {
    name: "Pool",
  ...
  pet: {
    ...
    icon: "mdi:dog",
    color: "var(--state-on-icon, var(--success-color, #20e3a2))",
    entity: "select.pet_feeder_mode",
    error_entity: "binary_sensor.pet_feeder_error",
    status_entities: [
      "input_select.pet_feeder_morning_status",
      "input_select.pet_feeder_midday_status",
      "input_select.pet_feeder_evening_status",
    ],
    navigation_path: "/lovelace/pet",
  },
  security: {
    name: "Sikkerhed",
    ...
    lock_entities: ["lock.utility_door", "lock.front_door", "lock.garage_door"],
    ...
    alarm_entity: "alarm_control_panel.primary_alarm",
    secondary_alarm_entity: "alarm_control_panel.secondary_alarm",
    door_entities: [
      "binary_sensor.stue_terrassedor",
      "binary_sensor.hoveddoren_contact",
      "binary_sensor.bryggersdor_contact",
      "binary_sensor.bedroom_terrace_door",
...
const SUMMARY_DEFAULTS = {
  ...
  electricity_price_entity: "sensor.stromligning_current_price_vat",
  electric_month_cost_entity: "",
  water_month_entity: "sensor.vandmaler_manedens_forbrug",
  ...
  rooms: [
    ...
    { name: "Værelse 1", icon: "mdi:bed-single-outline", temperature: "sensor.bedroom_1_temperature", humidity: "sensor.bedroom_1_humidity", climate: "climate.bedroom_1" },
    { name: "Værelse 2", icon: "mdi:bed-single-outline", temperature: "sensor.bedroom_2_temperature", humidity: "sensor.bedroom_2_humidity", climate: "climate.bedroom_2" },
  ...
  calendars: [
    { entity: "calendar.household", name: "Fælles", color: "var(--orange, #fb923c)" },
    { entity: "calendar.person_1", name: "Person 1", color: "var(--yellow, #facc15)" },
    { entity: "calendar.person_2", name: "Person 2", color: "var(--green, #22c55e)" },
    { entity: "calendar.work", name: "Arbejde", color: "var(--dashboard-accent, #38bdf8)" },
    { entity: "calendar.activity", name: "Aktivitet", color: "var(--purple, #a855f7)" },
  ],
};
...
class HaHomeDesktopLayoutCard extends HTMLElement {
  ...
  setConfig(config){
    ...
    this.config=structuredClone(config);this.config.header_cards=Array.isArray(config.header_cards)?structuredClone(config.header_cards):[];this.config.mobile_cards=Array.isArray(config.mobile_cards)?structuredClone(config.mobile_cards):[];this._build();
  }
  ...
  async _build(){
    ...
    this.shadowRoot.innerHTML=`<style>:host{display:block}.header,.mobile{display:flex;flex-direction:column;min-width:0;gap:var(--desktop-gap,clamp(10px,.75vw,18px))}.header:not(:empty){margin-bottom:var(--desktop-gap,clamp(10px,.75vw,18px))}.layout{display:grid;grid-template-columns:var(--desktop-columns,minmax(0,.9fr) minmax(440px,1.1fr));align-items:stretch;height:var(--desktop-height,auto);min-height:0;overflow:hidden;gap:var(--desktop-gap,clamp(10px,.75vw,18px))}.column{display:flex;flex-direction:column;min-width:0;min-height:0;overflow:hidden;gap:var(--desktop-gap,clamp(10px,.75vw,18px))}.slot{min-width:0;flex:0 0 auto}.slot.grow{display:flex;flex:1 1 0;min-height:0;overflow:hidden}.slot.grow>*{flex:1;min-width:0;min-height:0}.mobile{display:none}@media(max-width:1399px) and (min-width:1101px){.layout{grid-template-columns:minmax(0,1fr) minmax(420px,1fr)}}@media(max-width:1100px){.header:not(:empty){margin-bottom:8px}.layout{display:none}.mobile{display:flex;gap:8px}}</style><div class="header"></div><div class="layout"><div class="column left"></div><div class="column right"></div></div><div class="mobile"></div>`;
    const make=(cfg,parent,index,total,growLast=false)=>{const card=helpers.createCardElement(cfg);const slot=document.createElement("div");slot.className=`slot${growLast&&index===total-1?" grow":""}`;slot.append(card);parent.append(slot);this._children.push(card);if(this._hass)card.hass=this._hass;};
    this._children=[];const header=this.shadowRoot.querySelector(".header"),left=this.shadowRoot.querySelector(".left"),right=this.shadowRoot.querySelector(".right"),mobile=this.shadowRoot.querySelector(".mobile");
    ...
    this.config.mobile_cards.forEach((cfg,i)=>make(cfg,mobile,i,this.config.mobile_cards.length));
  ...
  _fitViewport(){const layout=this.shadowRoot?.querySelector(".layout");if(!layout||!window.matchMedia("(min-width:1101px)").matches)return;const top=layout.getBoundingClientRect().top;const viewportHeight=window.visualViewport?.height||window.innerHeight;const bottomGap=Math.max(90,Number(this.config?.bottom_gap)||110);layout.style.setProperty("--desktop-height",`${Math.max(0,viewportHeight-top-bottomGap)}px`);}
  getCardSize(){return 12;}
  static getConfigElement(){return document.createElement("ha-home-desktop-layout-card-editor");}
  static getStubConfig(){return{columns:"minmax(0,.9fr) minmax(440px,1.1fr)",gap:"clamp(10px,.75vw,18px)",bottom_gap:110,header_cards:[],left_cards:[],right_cards:[],mobile_cards:[]};}
}
class HaHomeDesktopLayoutCardEditor extends HTMLElement {
  constructor(){super();this.attachShadow({mode:"open"});this._nestedEditors=new Set();}
  setConfig(config){this.config=structuredClone(config);this.render();}
  ...
  _emit(config){this.config=structuredClone(config);this.dispatchEvent(new CustomEvent("config-changed",{bubbles:true,composed:true,detail:{config:this.config}}));}
