  ...
  _render() {
    ...
    if (!this._rendered) {
      this.shadowRoot.innerHTML = `<style>
        ...
        .body{display:grid;grid-template-columns:minmax(0,1.42fr) minmax(205px,.58fr);flex:1;min-height:0;gap:12px}.panel{padding:13px;border-radius:16px;background:color-mix(in srgb,var(--black,#000) 13%,transparent);border:1px solid color-mix(in srgb,var(--primary-text-color,#fff) 8%,transparent)}.panel.clickable{cursor:pointer;transition:border-color .18s ease,background .18s ease,transform .18s ease}.panel.clickable:hover,.panel.clickable:focus-visible{border-color:color-mix(in srgb,var(--dashboard-accent,#38bdf8) 48%,transparent);background:color-mix(in srgb,var(--dashboard-accent,#38bdf8) 6%,transparent);outline:none}.panel.clickable:active{transform:scale(.995)}.temperature-panel{position:relative}.panel h3{font-size:12px;text-transform:uppercase;letter-spacing:.08em;margin:0 0 10px;color:var(--secondary-text-color,#a7b2c2)}
      ...
      const openPanel=(selector,pathKey)=>{const panel=this.shadowRoot.querySelector(selector);panel.onclick=()=>navigate(this.config[pathKey]);panel.onkeydown=(event)=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();navigate(this.config[pathKey]);}};};
      openPanel(".temperature-panel","rooms_navigation_path");
      openPanel(".agenda-panel","calendar_navigation_path");
      this._rendered = true;
    }
...
class HaHomeDesktopLayoutCard extends HTMLElement {
  ...
  setConfig(config){
    ...
    this.config=structuredClone(config);this.config.header_cards=Array.isArray(config.header_cards)?structuredClone(config.header_cards):[];this.config.mobile_cards=Array.isArray(config.mobile_cards)?structuredClone(config.mobile_cards):[];this.config.vertical_cards=Array.isArray(config.vertical_cards)?structuredClone(config.vertical_cards):[];this._build();
  }
  ...
  async _build(){
    ...
    const make=(cfg,parent,index,total,growLast=false,extraClass="")=>{const card=helpers.createCardElement(cfg);const slot=document.createElement("div");slot.className=`slot${growLast&&index===total-1?" grow":""}${extraClass?` ${extraClass}`:""}`;slot.append(card);parent.append(slot);this._children.push(card);if(this._hass)card.hass=this._hass;};
