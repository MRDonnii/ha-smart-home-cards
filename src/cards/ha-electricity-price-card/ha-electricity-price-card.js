...
class HAElectricityPriceCard extends HTMLElement {
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `<style>
      ...
      .tabs{padding:4px;gap:4px;border:1px solid var(--divider-color,var(--edge));border-radius:14px;background:var(--card-background-color,var(--surface-local));box-shadow:var(--dashboard-shadow-soft,var(--ha-card-box-shadow,none))}.tabs button{border:0;border-radius:10px;background:transparent;box-shadow:none;backdrop-filter:none;-webkit-backdrop-filter:none}.tabs button:hover{color:var(--primary-text-color);background:var(--contrast1,color-mix(in srgb,var(--primary-text-color) 4%,transparent))}.tabs button.active{border:0;color:var(--primary-text-color);background:var(--dashboard-tab-selected-bg,color-mix(in srgb,var(--accent) 14%,transparent));box-shadow:inset 0 0 0 1px var(--dashboard-tab-selected-border,var(--accent)),0 0 18px -8px var(--accent)}
