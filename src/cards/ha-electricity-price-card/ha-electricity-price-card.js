...
class HAElectricityPriceCard extends HTMLElement {
  ...
  _render() {
    ...
    card.style.height = compactMobile ? (this._config.show_header === false ? "345px" : "390px") : fillHeight ? "100%" : `${desktopHeight}px`;
    ...
    if (chart) {
      if (fillHeight) {
        ...
        chart.style.height = `${169 + headerGain}px`;
        chart.style.paddingTop = `${22 + Math.round(headerGain * 0.28)}px`;
        this.shadowRoot.querySelectorAll(".bar-wrap").forEach((bar) => { bar.style.height = `${146 + Math.round(headerGain * 0.72)}px`; });
      }
