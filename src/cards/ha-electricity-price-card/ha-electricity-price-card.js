  ...
  _render() {
    ...
    if (chart) {
      if (fillHeight) {
        ...
        chart.style.height = `${151 + extraHeight + headerGain}px`;
        chart.style.paddingTop = `${34 + Math.round((extraHeight + headerGain) * 0.28)}px`;
        this.shadowRoot.querySelectorAll(".bar-wrap").forEach((bar) => {
          bar.style.height = `${116 + Math.round((extraHeight + headerGain) * 0.72)}px`;
        });
      } else if (!compactMobile && headerGain) {
        chart.style.height = `${151 + headerGain}px`;
        chart.style.paddingTop = `${34 + Math.round(headerGain * 0.28)}px`;
        this.shadowRoot.querySelectorAll(".bar-wrap").forEach((bar) => { bar.style.height = `${116 + Math.round(headerGain * 0.72)}px`; });
      } else if (compactMobile && headerGain) {
        chart.style.height = `${148 + headerGain}px`;
        chart.style.paddingTop = `${33 + Math.round(headerGain * 0.28)}px`;
        this.shadowRoot.querySelectorAll(".bar-wrap").forEach((bar) => { bar.style.height = `${114 + Math.round(headerGain * 0.72)}px`; });
      }
