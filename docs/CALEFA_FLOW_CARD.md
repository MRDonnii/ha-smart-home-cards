...
- **Right:** Varme frem / Varme retur with the ΔT ring between them, Bypass, Varmt vand and Koldt vand.
...
| --- | --- | --- |
| FF: outer left riser (90, 59, strainer 53) and supply line to the supply tee with 51 FJF | orange | either primary branch moves |
| Supply tee → up behind the manifold → diagonal into the heating exchanger's upper port | orange | heating valve 22/34 open (`heating_valve` above `valve_threshold`) |
| Supply tee → outer U-bend → DHW exchanger's lower port | orange | DHW valve 37 open (`dhw_valve` above `valve_threshold`): tapping or bypass |
| Heating exchanger's lower port → heating valve (22/34) → return manifold | blue | heating valve open |
| DHW exchanger's upper port → 51 FJR → 37 control valve → return manifold | blue | DHW valve open: tapping or bypass |
| FR: return manifold (joins downstream of 22/34), 52 and second riser | blue | either primary branch moves |
| VR through the pump (40) and strainer (53), over the inner U-bend into the heating exchanger's lower port | light blue | pump running (`pump`); `heating_flow` only without a pump entity |
| VF from the heating exchanger's upper port, behind the valves and U-bends, through the safety valve (25) | orange | pump running; `heating_flow` only without a pump entity |
...
Without a valve reading, a branch falls back to the old signals: tapping for the DHW branch and `heating_active` with a measured `fjv_flow` for the heating branch. `fjv_flow` (for example a heat meter that reports every 1–2 minutes) never keeps a branch moving after its valve has closed and never starts one; it only sets the arrow speed when a valve reading is missing. The arrow speed of a branch follows its valve opening in four steps, the hot water side follows `water_flow` and the trunk follows the faster branch.

### Bypass

Wavin's principle diagram for the Calefa II V ITC (installation guide, October 2025, p. 5) has no separate bypass pipe and no thermostatic bypass valve. The electronic bypass is the DHW branch with valve 37 held slightly open while no hot water is drawn: FF → 90 → 59 → 53 → supply tee with 51 FJF → DHW exchanger (01) primary side from the bottom up → 51 FJR → 37 with stepper motor 34 → down across the supply line **without a joint** → tee on the return line downstream of 22/34 → 52 → 90 → 59 → FR. The cold and hot water side of 01 stands still. The sensor colours on p. 4 place FJF (red) in the supply tee and FJR (dark blue) on the pipe into 37, so the FJV retur callout points there and FJV ΔT is the cooling across the DHW branch.

| Bypass state | Condition | Drawing |
| --- | --- | --- |
| Kører | `dhw_active` reports Bypass, no tapping and `dhw_valve` above `valve_threshold` | bypass route with thin, slow arrows (faster above 5 %), warm bottom of the DHW exchanger, tile *Bypass · Kører*, BV-ventil *Bypass* |
| Klar | `dhw_active` reports Bypass but valve 37 is closed (or has no reading) | bypass route dotted and still, tile *Bypass · Klar*, BV-ventil *Bypass klar* |
| Fra | any other DHW state | no bypass drawing, grey tile *Bypass · Fra* |

The bypass state of the integration (`BVV bypass`, `dhw_bypass_active`) is derived from the DHW state and is not a flow measurement. On the reference installation a bypass state lasted up to an hour with valve 37 at 0 %, while real bypass flow only appeared when the valve opened: about 160 L/h during a short 8 % opening and 3–30 L/h during 1.5–2.5 % ramps (measured by the heat meter with heating and tapping off). The tile shows the bypass mode in the controller's words and its temperature from `bypass_mode` and `bypass_temperature` (for example *Auto · 40 °C*); tapping it opens `dhw_active`. The display LED keeps the controller's own meaning: slow cyan while the bypass state is active.

Moving arrows run along active tracks; their speed follows the measured flow in four steps. Each exchanger face carries a soft fog layer clipped to its measured outline, coloured from the live temperatures (blue when cold, pale in between, orange to red when hot) with slow wisps that follow the district heating flow. The front DHW exchanger is hot at the bottom and cold at the top while tapping and glows only at the bottom while a bypass flows; the heating exchanger's visible top strip is its hot end and fades from FJV supply towards FJV return while its heating valve is open. No fog is shown for an armed bypass with a closed valve.
...
`flow_threshold` defaults to `0.05`; `valve_threshold` defaults to `1`. Explicit `heating_active` / `dhw_active` entities decide the heating and hot water state; a positive `water_flow` always counts as tapping. Valve positions decide which primary branch carries water. `show_footer: false` hides the footer. FJV ΔT equals FJV supply minus FJV return; heating ΔT equals heating supply minus heating return. Each pair shares a circular ΔT badge: green means at or above its configured cooling target, red means below, and neutral means a reading is missing. Defaults are `fjv_good_delta: 20` and `heating_good_delta: 5` °C; both can be adjusted in card configuration.
